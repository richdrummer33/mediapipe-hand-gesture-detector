...
def check_ok_gesture(image, hand_landmarks):
    ...
    if thumb_index_dist < 0.05:
        ...
        if index.y > lowest_other_finger.y and normalized_max_dist_to_others > 1:
            print(f"normalized_max_dist_to_others: {normalized_max_dist_to_others}")
...
def check_start_gesture(gesture, on_ok_gesture):
    ...
    if gesture == "Victory" and not gesturing:
        ...
        gesturing = True
        if on_ok_gesture is not None:
            on_ok_gesture()

    elif gesture is None:
        gesturing = False

def check_end_gesture(on_pinch_gesture=None):
    ...
    if debounce_timer < 0:
        ...
        pyautogui.hotkey('alt', 'm')
        gesturing = False
        if on_pinch_gesture is not None:
            on_pinch_gesture()

# Main function
class HandGestureDetector:
    def __init__(self, on_ok_gesture, on_pinch_gesture):
        self.on_ok_gesture = on_ok_gesture
        self.on_pinch_gesture = on_pinch_gesture
    ...
    def run(self):
        global debounce, debounce_timer, last_time

        with mp.solutions.hands.Hands(min_detection_confidence=0.3, min_tracking_confidence=0.3) as hands, mp.solutions.face_mesh.FaceMesh() as face_mesh:
            while self.cap.isOpened():
                ...
                if x_min is not None and hand_landmarks is not None:
                    ...
                    if gesture_status == "OK": # num_fingers_on_face == 2:
                        check_start_gesture("Victory", on_ok_gesture=self.on_ok_gesture)
        ...
        # exited the while loop (presumably on ctrl-c pressed)
        cap.release()
        cv2.destroyAllWindows()

if __name__ == "__main__":
    on_ok_action = lambda: print("Gesture action: Start")
    on_pinch_action = lambda: print("Gesture action: End")
    detector = HandGestureDetector(on_ok_action, on_pinch_action)
    detector.run()
